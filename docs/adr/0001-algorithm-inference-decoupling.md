...
Issue `intellistream/sageRefiner#15` requires that refiner keeps algorithm logic only and does not
carry inference-service binding conventions.

Audit findings:

- `LongLLMLinguaCompressor` exposed `open_api_config` and passed it into `PromptCompressor`
  initialization.
- `LongRefiner` prompt template contained OpenAI-specific branch flags (`is_openai`) and branch
  logic.
...
1. Remove OpenAI-specific prompt-branch logic from `LongRefiner` prompt template.
   - Delete `is_openai` state and all dependent branches.
1. Keep a single canonical algorithm path and fail-fast behavior in algorithm modules.
1. Add regression tests to lock this boundary.
