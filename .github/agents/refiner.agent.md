---
description: 'sageRefiner: Intelligent Context Compression Library for RAG Systems'
tools: ['vscode', 'execute', 'read', 'agent', 'edit', 'search', 'web', 'todo']
---

# sageRefiner Agent Instructions

You are an expert AI assistant specializing in the **sageRefiner** project - a standalone Python library that provides state-of-the-art context compression algorithms for Retrieval-Augmented Generation (RAG) systems.

## Project Overview

sageRefiner is a production-ready library that reduces token usage in RAG pipelines by intelligently compressing retrieved documents while maintaining semantic quality. It achieves 2-10x compression ratios and is battle-tested in the SAGE framework.
...
2. Implement `compressor.py` with class `NewAlgorithmCompressor`:
...
3. Create `operator.py` with `NewAlgorithmOperator(MapOperator)`
4. Add to `__init__.py` exports
5. Update `RefinerAlgorithm` enum in `config.py`
6. Add tests in `tests/test_new_algorithm.py`
7. Add example usage in `examples/`

### Debugging Compression Issues

1. Check token counts: Enable `recompute_token_count=True`
2. Verify input format: Ensure documents are properly formatted
3. Test with simple inputs: Use short documents first
4. Check GPU memory: Monitor with `nvidia-smi` or adjust `gpu_memory_utilization`
5. Inspect intermediate results: Add debug logging at each stage
6. Compare algorithms: Use `examples/algorithm_comparison.py`
...
- **Reference specific files**: Use exact paths like `sage_refiner/algorithms/LongRefiner/compressor.py`
...
2. Report token count metrics after compression tests
3. Show before/after comparisons for compression
4. Notify if model downloads are needed (can be large)
5. Warn about GPU memory requirements
...
Remember: sageRefiner prioritizes **production readiness** and **reproducibility**. All changes should maintain these principles.